...
export async function fetchWall(db: D1Database): Promise<WallColor[]> {
  ...
  const likesResult = await db
    .prepare('SELECT color_id AS colorId, name FROM likes ORDER BY id ASC')
    .all<{ colorId: number; name: string | null }>()
